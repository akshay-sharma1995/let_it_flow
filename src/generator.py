...
class gen(nn.Module):
    def __init__(self, input_channels= 2, latent_dim=108, out_channels=2, lrEnc=1e-3, lrDec=1e-3):
        ...
        self.Encoder = Encoder(self.input_channels, self.latent_dim, self.out_channels, self.lrEnc)
        self.Decoder = Decoder(self.Encoder.encoded_dim, self.input_channels, self.latent_dim, self.out_channels, self.lrDec)
    ...
    def forward(self, x):
        ...
        noise_params = self.Encoder.latent_code(encoded_vec)
        mean, logvar = noise_params[:,0:self.latent_dim], noise_params[:,self.latent_dim:]
        sample = self.sample_noise(mean, logvar)
        
        decoded_latent_code = self.Decoder.latent_code_decoder(sample)
...
class Encoder(nn.Module):
    ...
    def __init__(self, input_channels= 2, latent_dim=108, out_channels=2, lr=1e-3):
        ...
        self.encoded_dim = 16 * 27 * 9

        self.latent_code = nn.Sequential(
                                        nn.Linear(in_features= self.encoded_dim, out_features=self.encoded_dim//9, bias=True),
                                        nn.ReLU(),
                                        nn.BatchNorm1d(self.encoded_dim//9),
                                        
                                        nn.Linear(in_features=self.encoded_dim//9, out_features=2*latent_dim, bias=True),
                                        )

        self.Encoptimizer = torch.optim.Adam(self.parameters(), lr=lr)
...
class Decoder(nn.Module):
    ...
    def __init__(self, encoded_dim , input_channels= 2, latent_dim=108, out_channels=2, lr=1e-3):
        ...
        self.Decoptimizer = torch.optim.Adam(self.parameters(), lr=lr)
